...
def meeting_list_api(request):
    ...
    qs = (
        Meeting.objects.filter(finished_at__gt=now)
    ...
    qs = qs.order_by("-views", "-id") if sort == "views" else qs.order_by("started_at", "-id")
    if limit:
        try: qs = qs[:int(limit)]
        except: pass
...
def meeting_quiz_api(request, pk):
    ...
    is_correct = (user_answer == normalize_answer(quiz.answer))
